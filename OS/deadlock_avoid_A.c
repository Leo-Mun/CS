...
⁃	철학자 세명 A,B,C
⁃	젓가락 세개 R1,R2,R3
⁃	각 젓가락을 Mutual Exclusive 하게 만들기 위한 Lock L1
...
void Take_R1() {
   ...
   // grab chopstick if :
   // not last fork
   // last fork and requesting thread needs only one more fork
   // last fork, but there is some thread who has two forks
   // else wait

   while (Load("R1") == 1 ? ((Load("R2") + Load("R3")) != 0) ? 0 : (((Load("ACount") == 1) || (Load("BCount") == 2 || Load("CCount") == 2)) ? 0 : 1) : 1) {
...
void Take_R2() {
   ...
   // last fork, but there is some thread who has two forks
   // else wait
   while (Load("R2") == 1 ? ((Load("R1") + Load("R3")) != 0) ? 0 : (((Load("ACount") == 1) || (Load("BCount") == 2 || Load("CCount") == 2)) ? 0 : 1) : 1) {
...
void Take_R3() {
   ...
   while (Load("R3") == 1 ? 0 : ((Load("R1") + Load("R2")) != 0) ? 0 : (Load("ACount") == 1) ? 0 : (Load("BCount") == 2 || Load("CCount") == 2) ? 0 : 1) {
