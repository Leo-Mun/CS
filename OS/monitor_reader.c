...
// fileVar 화일의 마지막 값을 읽어 온다.
  ...
  // variables just to store last pid, time
  long tmp2;
  int tmp;
...
void main(int argc, char *argv[]) {
   ...
   // Reader (First check self into system)
   Acquire(&lock);   // lock.Acquire()
   printf("process %d in critical section\n",pid);
   // while waiting if writer waits or doing write
   ...
   Acquire(&lock);
   sub("AR", 1); // AR--;
   // no write thread exists, wake up one waiting writer thread
   if (Load("AR") == 0 && Load("WW") > 0) {
    Signal(&okToWrite); // okToWrite.signal();
   }
